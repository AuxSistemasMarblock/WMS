-- 0002_crear_caso_rpc.sql
-- Función transaccional para crear un caso de justificación.
--
-- ¿Por qué una función (RPC)? La creación de un caso implica 3 escrituras
-- relacionadas (insert caso -> update discrepancias -> insert eventos). Hacerlo
-- como 3 llamadas REST desde el backend no es atómico: un fallo intermedio deja
-- un caso huérfano o discrepancias en_revision sin caso. Esta función ejecuta
-- todo en UNA transacción, por lo que o se aplica completo o no se aplica nada.
--
-- Acceso: el backend la invoca con la service role key (supabase.rpc). Se revoca
-- EXECUTE de public/anon/authenticated para que no sea invocable desde el cliente.
...
begin
  ...
  -- Sucursal: la indicada o la de la primera discrepancia
  v_sucursal := coalesce(
    p_sucursal,
    (select d.sucursal from public.discrepancias d where d.id = p_discrepancia_ids[1])
  );

  -- Folio CASO-YYYY-NNNN serializado por año (advisory lock transaccional)
...
-- Solo el backend (service role) puede invocarla.
