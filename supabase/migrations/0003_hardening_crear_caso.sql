-- 0003_hardening_crear_caso.sql
-- Endurecimiento de seguridad del gestor de casos.
--
-- 1) `crear_caso` quedaba con search_path mutable (lint function_search_path_mutable),
--    lo que permite ataques de hijacking de search_path. Se fija a '' y el cuerpo
--    ya usa referencias calificadas (public.*).
-- 2) Las tablas nuevas, aunque tienen RLS habilitado sin políticas, seguían siendo
--    visibles en el esquema GraphQL para anon/authenticated. Se revoca SELECT a esos
--    roles (el backend usa service_role, que no se ve afectado).
...
-- Ocultar las tablas nuevas del esquema GraphQL (anon/authenticated).
revoke select on public.tipos_justificacion from anon, authenticated;
revoke select on public.casos               from anon, authenticated;
revoke select on public.discrepancias       from anon, authenticated;
revoke select on public.caso_eventos        from anon, authenticated;
